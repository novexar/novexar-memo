# 04 Snowflake Cortex 編
### AISQL / Cortex Search / Cortex Analyst / Cortex Agents / マルチエージェント（Agent Toolset）

> **この巻で分かること**
> - 「Cortex」という言葉が指す機能群の全体像と、その積み上がり方
> - SQL から LLM を呼ぶ AISQL の使い方と、バッチ／対話の使い分け
> - 非構造データを検索する Cortex Search（RAG の検索側）の作り方
> - 自然言語から SQL を生成する Cortex Analyst と、その精度を決める Semantic View
> - ツールを選んで実行する Cortex Agents と、マルチエージェント構成
>
> **前提知識**
> - `01-snowflake-basics.md`: オブジェクト階層、ウェアハウス、コスト（Cortex は全部この上に乗ります）
> - `02-snowflake-data-engineering.md`: Dynamic Table の増分更新（Cortex Search のリフレッシュがこれに従います）
> - `03-snowflake-governance-dev.md`: RBAC（Agents の権限判定がこれに従います）
>
> 出典: Snowflake 公式ドキュメント（docs.snowflake.com）および Snowflake 公式ブログ／作成日: 2026-08-04

---

## 0. このドキュメントの使い方

本書は Snowflake 学習ドキュメントセットの 4 冊目です。**01〜03 を先に通してから読むことを強く推奨します**（理由は `00-README.md` に記載）。各章の末尾に **「手を動かすチェックリスト」** があるので、トライアルアカウントで実際に実行しながら進めてください。

記号の意味は全巻共通です。📘 用語 ／ 💡 なぜ？ ／ ⚠️ つまずきポイント ／ ❓ よくある疑問 ／ ✅ 手を動かす ／ 🏢 実務メモ
...
初学者が最初につまずくのは「Cortex ＝ 何を指すのか」が曖昧な点です。**Cortex は単一の製品名ではなく、Snowflake の AI 機能群の総称**です。「AWS」と同じで、その下にいくつもの別々のサービスがぶら下がっています。

📘 **前提となる 3 つの用語**

| 用語 | 意味 |
|---|---|
| **LLM（大規模言語モデル）** | 文章を入力すると、続きや答えを生成するモデル。GPT、Claude、Llama など |
| **埋め込み（Embedding）** | テキストを数百〜数千次元の数値ベクトルに変換したもの。意味が近い文章はベクトルも近くなる |
| **RAG（検索拡張生成）** | 「まず関連文書を検索し、その内容を LLM に渡して答えさせる」手法。社内文書に基づく回答の定番構成 |

💡 **Snowflake で AI を動かす最大の利点**
モデルが Snowflake 内でホストされているため、**データを外部に持ち出さずに推論できます**。外部の API に投げる構成だと、データ持ち出しの可否がプロジェクト初期の最大の障壁になりますが、Cortex ではその論点自体が消えます。既存の RBAC もそのまま効きます。

以上を踏まえて、Cortex は**下から積み上がる 4 層**として理解すると整理できます。
...
❓ **よくある疑問: Cortex Analyst と Cortex Agents はどう違うの？**
Analyst は**道具**、Agents は**その道具を使う人**です。Analyst 単体は「質問 → SQL → 結果」しかできません。「規程を検索してから数値も出して、グラフにして」のように**複数の道具を組み合わせて段取りを考える**のが Agents です。まず Analyst と Search を作り、それらを束ねるのが Agents、という順序で理解してください。

❓ **よくある疑問: Snowflake Intelligence とは？**
上記のすべてをノーコードのチャット画面で使えるようにした UI です。Snowsight から使えます。**エージェントを SQL で作らなくても、画面上で作って試せる**ため、最初の学習には便利です。本ドキュメントでは仕組みを理解するため SQL 中心で説明します。

---

## 2. 事前準備

> **この章のゴール**: 「権限がない」「リージョンで使えない」という 2 大つまずきを、着手前に潰しておくこと。

💡 **Cortex 特有の権限モデル**
Cortex の機能は `SNOWFLAKE` という**システムデータベースが提供するデータベースロール**によって制御されます。通常のテーブル権限（03 巻）とは別枠なので、「テーブルは見えるのに AI 関数だけエラーになる」ということが起こります。
...
❓ **よくある疑問: どのロールを付ければいいか分からない**
学習段階では `SNOWFLAKE.CORTEX_USER` を 1 つ付けておけば、AISQL・Search・Analyst・Agents のすべてが動きます。本番では機能別のロール（`CORTEX_ANALYST_USER` など）に絞って最小権限にします。
...
> **この章のゴール**: 「SQL 関数として AI を呼ぶ」という発想に慣れること。難しい概念はありません。

💡 **AISQL の考え方**
`UPPER(col)` や `SUM(col)` と同じ感覚で、`AI_CLASSIFY(col, [...])` と書けます。**AI 処理が SQL の一部になる**ため、既存のデータパイプラインにそのまま組み込めます。10万件のレビューを分類したいなら、`UPDATE` 文 1 本で終わります。Python を書いて API を叩いてループを回す、という作業が不要になります。

```mermaid
flowchart LR
    T["テーブル<br/>REVIEWS"] --> F["AI_CLASSIFY(body, [...])"]
    F --> R["結果が列として返る"]
    R --> U["そのまま GROUP BY / WHERE / JOIN できる"]
...
⚠️ **つまずきポイント: いきなり全件に実行しない**
100万行のテーブルに `AI_COMPLETE` を投げると、費用も時間も予想外に膨らみます。**必ず `LIMIT 100` で試し、消費クレジットを確認してから全件に広げてください。**

```sql
-- 実行前にトークン数を見積もる
SELECT SUM(AI_COUNT_TOKENS('claude-4-sonnet', body)) AS total_input_tokens
FROM reviews;
```

❓ **よくある疑問: どのモデルを選べばいい？**
迷ったら小〜中型モデルから始めてください。分類・抽出・要約のような**定型処理は小型モデルで十分な精度が出ることが多く**、大型モデルとの費用差は数倍〜十数倍あります。品質が足りないと判断してから上げるのが正しい順序です。

❓ **よくある疑問: 同じ入力でも結果が変わる**
LLM は確率的に出力するため、完全な再現性はありません。分類のように安定性が必要な用途では、`AI_CLASSIFY` のようなタスク特化関数を使い、カテゴリを明示的に与えてください。自由生成より遥かに安定します。
...
> **この章のゴール**: RAG の「検索側」を Snowflake だけで完結させられるようになること。

💡 **そもそも、なぜ検索が必要なのか**
LLM に「うちの返品ポリシーは？」と聞いても、社内文書を学習していないので答えられません。そこで **①関連しそうな文書を検索し、②その文書と質問をまとめて LLM に渡す**、という 2 段構えにします。これが RAG です。Cortex Search は①を担当します。

```mermaid
flowchart LR
    Q["質問<br/>返品はいつまで？"] --> S["① Cortex Search<br/>関連文書を検索"]
    D["社内文書<br/>（テーブルに格納）"] --> S
    S --> C["② 検索結果 ＋ 質問を<br/>LLM に渡す"]
    C --> A["③ 根拠付きの回答"]

    style S fill:#e8f5e9,stroke:#28a745,stroke-width:2px
...
| 用語 | 意味 |
|---|---|
| **チャンク（Chunk）** | 長い文書を検索しやすい大きさに切り分けた断片。切り方が精度を左右する |
| **ベクトル検索** | 埋め込みベクトルの近さで探す。「返品」で「返却・キャンセル」もヒットする |
| **キーワード検索** | 字面の一致で探す。型番や固有名詞に強い |
| **ハイブリッド検索** | 上記 2 つを組み合わせる。Cortex Search はこれを自動で行う |
| **リランキング** | 検索結果を意味的な関連度で並べ替える最終工程 |
...
❓ **よくある疑問: 元の文書が PDF や Word の場合は？**
まず `AI_PARSE_DOCUMENT` でステージ上のファイルからテキストを抽出し、`SPLIT_TEXT_RECURSIVE_CHARACTER` でチャンクに分割してテーブルに入れます。その上で Cortex Search サービスを作ります。

```sql
-- ① ステージ上のPDFからテキストを抽出
CREATE OR REPLACE TABLE doc_text AS
SELECT relative_path,
       AI_PARSE_DOCUMENT(TO_FILE('@doc_stage', relative_path),
...
FROM doc_text,
     LATERAL FLATTEN(input => SNOWFLAKE.CORTEX.SPLIT_TEXT_RECURSIVE_CHARACTER(body, 'markdown', 1500, 200)) c;
```

❓ **よくある疑問: 検索結果が的外れになる**
原因の多くは次の 3 つです。① 英語専用の埋め込みモデルを日本語データに使っている、② チャンクが大きすぎて 1 チャンクに複数の話題が混在している、③ `ATTRIBUTES` にフィルタ列を指定しておらず、絞り込みができていない。まず①を確認してください。
...
> **この章のゴール**: 「なぜ Text-to-SQL は素朴にやると精度が出ないのか」を理解し、Semantic View で解決できるようになること。**Cortex の中で最も実務価値が高く、最も設計力が問われる部分です。**

💡 **なぜ「LLM に SQL を書かせる」だけでは失敗するのか**

テーブル定義だけを LLM に見せて「先月の売上は？」と聞くと、次のような問題が起きます。

```mermaid
flowchart TB
    Q["質問: 先月の売上は？"] --> LLM["LLM"]
    SC["スキーマ情報だけ<br/>ORDERS(O_TOTALPRICE, O_ORDERDATE...)"] --> LLM
    LLM --> P1["❓ 売上 = O_TOTALPRICE？<br/>それとも明細の合計？"]
    LLM --> P2["❓ 割引後？割引前？"]
    LLM --> P3["❓ キャンセル分は除く？"]
    LLM --> P4["❓ 先月 = 暦月？会計月？"]
    P1 --> W["それらしいが間違った SQL"]
...
**列名だけではビジネスの意味が分からない**からです。これはモデルを大きくしても解決しません。**人間の新人が入社初日に同じ質問をされても答えられないのと同じ**で、必要なのは能力ではなく業務知識の提供です。

その業務知識を機械可読な形で与えるのが **Semantic View** です。

```mermaid
flowchart LR
    T["物理テーブル<br/>列名・型・PK"] --> SV["Semantic View"]
    B["ビジネス知識<br/>・売上の定義<br/>・日本語のシノニム<br/>・使ってよい値<br/>・検証済みクエリ"] --> SV
    SV --> CA["Cortex Analyst"]
    CA --> SQL["正しい SQL"]

    style SV fill:#e8f5e9,stroke:#28a745,stroke-width:2px
...
📘 **Fact / Dimension / Metric の違い（初学者が最も混乱する箇所）**

| | 意味 | 判定のコツ | 例 |
|---|---|---|---|
| **Dimension** | 「何で切るか」の軸 | GROUP BY に書く側 | 顧客名、受注年、地域 |
| **Fact** | 行単位の生の数量 | まだ集計していない値 | 明細1行の金額 |
| **Metric** | 集計した KPI | SUM / AVG など集約関数が付く | 売上高、平均受注額 |

**「地域別の売上高」なら、地域＝Dimension、売上高＝Metric** です。Fact は Metric を組み立てるための材料と考えてください。
...
⚠️ **ここが Analyst 実務の 8 割です。** 「精度が出ない」という相談のほとんどは、Semantic View に情報が足りていないだけです。モデルの問題ではありません。
...
❓ **よくある疑問: 生成された SQL が間違っていたらどうする？**
`AI_VERIFIED_QUERIES` にその質問と正しい SQL のペアを登録してください。**同種の質問に対する精度が最も大きく改善する手段**です。運用としては「業務ユーザーが誤りを報告 → データスチュワードが検証済みクエリを追加」というループを回すのが定石です。

❓ **よくある疑問: 誰が Semantic View を作るべき？**
エンジニア 1 人では作れません。**「売上の定義」を決められるのは業務側**だからです。実務では、エンジニアが骨格（テーブル・リレーション）を作り、業務担当と一緒に COMMENT・シノニム・メトリクス定義を埋めていく進め方になります。この共同作業をスケジュールに組み込んでおかないと、必ず遅延します。

❓ **よくある疑問: 権限はどうなる？**
生成された SQL は**呼び出しユーザーのロール**で、自アカウントのウェアハウス上で実行されます。したがって 03 巻で設定した行アクセスポリシー・マスキングポリシーはそのまま効きます。**「見えないはずの行が Analyst 経由で見えてしまう」ことは起きません。**
...
> **この章のゴール**: 「エージェント＝ツールを選んで使う LLM」という基本構造を理解し、SQL で 1 体作れるようになること。

💡 **エージェントとは何か（3行で）**
これまでの章で作った Analyst と Search は、**それぞれ単機能の道具**でした。エージェントは、ユーザーの質問を読んで**どの道具をどの順で使うかを自分で決め、実行し、結果を見てさらに次を判断する** LLM です。人間でいえば「アシスタントに調べ物を頼む」感覚に近くなります。

```mermaid
flowchart LR
    U["「返品規程を確認して、<br/>返品件数の推移もグラフで」"] --> AG["Agent"]
    AG -->|"① 規程は検索だな"| T1["Cortex Search"]
    AG -->|"② 件数はSQLだな"| T2["Cortex Analyst"]
    AG -->|"③ グラフ化しよう"| T3["Data to Chart"]
    T1 --> AG
    T2 --> AG
    T3 --> AG
    AG --> R["引用付きの回答＋グラフ"]
...
| 用語 | 意味 |
|---|---|
| **ツール（Tool）** | エージェントが使える道具。Analyst、Search、Python 実行、独自関数など |
| **オーケストレーション** | どの道具をどう使うかを決める思考過程。自然言語の指示で方針を与えられる |
| **スレッド（Thread）** | 会話の記憶。これがあると「じゃあ大阪は？」が通じる |
| **Run** | 1 回の実行。内部では複数のツール呼び出しが起きうる |
...
💡 **仕様（SPECIFICATION）の読み方**
エージェントの定義は YAML で書きます。ブロックの役割は次の 4 つだけです。

| ブロック | 何を書くか |
|---|---|
| `models` | どの LLM で考えさせるか（`auto` 推奨） |
| `instructions` | 方針。`orchestration` ＝ 道具の使い分け方、`response` ＝ 回答の書き方 |
| `tools` | 使える道具の**宣言**（名前と種類と説明） |
| `tool_resources` | 各道具が**実際に使う対象**（Semantic View 名、検索サービス名、ウェアハウス等） |

⚠️ **`tools` の `name` と `tool_resources` のキーは必ず一致させてください。** ここの不一致が最も多い定義ミスです。
...
❓ **よくある疑問: エージェントが道具を使ってくれない**
`tool_spec.description` と `instructions.orchestration` が曖昧なことがほぼ唯一の原因です。LLM は description を読んで道具を選ぶため、**「いつ使う道具か」を description に具体的に書く**のが効きます。「売上データを検索する」ではなく「受注・売上・顧客に関する数値をSQLで集計する。金額・件数・推移の質問に使う」と書いてください。

❓ **よくある疑問: 開発はどこでやるのが効率的？**
Snowsight の **AI & ML » Agents » Playground** です。質問を投げると thinking ステップが表示され、**どの道具をどんな引数で呼んだか**が見えます。SQL で定義 → Playground で検証 → 指示を修正、というループが最速です。
...
> **この章のゴール**: 「PoC は安かったのに本番で跳ねた」を防ぐこと。

💡 **Cortex のコストは 3 種類が重なります**
① LLM の推論（トークン）、② 検索インデックスの維持（サイズ×時間）、③ 生成された SQL を実行するウェアハウス。**②は使っていなくても発生する**ため、見落とすと請求書で驚きます。

```mermaid
flowchart TB
    subgraph USE["使ったときだけ発生"]
        A["AISQL のトークン"]
        B["Analyst のメッセージ"]
        C["生成SQLの実行<br/>（ウェアハウス）"]
    end
    subgraph ALWAYS["使わなくても発生"]
        D["Cortex Search の<br/>サービングコスト<br/>（インデックスGB × 保持期間）"]
        E["ストレージ"]
    end
    style ALWAYS fill:#fdecea,stroke:#dc3545
    style USE fill:#e8f5e9,stroke:#28a745
```
...
> 以下は **01〜03 巻を終えた前提**の日程です。未着手なら、その分を先に確保してください。
